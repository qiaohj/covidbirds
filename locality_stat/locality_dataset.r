...
`OBSERVER ID` as OBSERVER_ID FROM eBird202012 \
...
item<-states[which(states$COUNTY=="Weston"),]
states<-states[sample(nrow(states), nrow(states)),]
...
for (i in c(1:nrow(states))){
  ...
  item<-states[i,]
  target<-sprintf("../../Tables/Locality_202012/%s/%s/%s", item$COUNTRY, item$STATE, item$COUNTY)
  ...
  loc_items<-locality%>%dplyr::filter((COUNTRY==item$COUNTRY)&(STATE==item$STATE)&
                                        (COUNTY==county)&(LOCALITY_ID!='P')&(LOCALITY_ID!='H'))
